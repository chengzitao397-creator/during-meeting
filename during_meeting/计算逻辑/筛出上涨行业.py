# -*- coding: utf-8 -*-
"""
从 会议窗口数据/ 下各窗口表（窗口_T-5.csv 等，已为一行一行业格式）中筛出「上涨的行业」，
合并为一张表：会议名称、T日、窗口、窗口开始、窗口结束、行业名称、上涨幅度、
上证指数涨跌幅、差值。结果写入 会议窗口数据/各窗口上涨行业.csv。
"""
import csv
from pathlib import Path


def run():
    base = Path(__file__).resolve().parent.parent
    dir_data = base / "会议窗口数据"
    windows = ["T-5", "T-1", "T", "T+1", "T+5"]
    out_rows = []
    out_fields = ["会议名称", "T日", "窗口", "窗口开始", "窗口结束", "市场涨跌幅", "上证指数涨跌幅", "行业名称", "上涨幅度", "差值"]

    for w in windows:
        path_csv = dir_data / f"窗口_{w}.csv"
        if not path_csv.exists():
            continue
        with open(path_csv, "r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                # 跳过空行（组间分隔行）
                if not (row.get("行业名称") or "").strip():
                    continue
                ret_str = (row.get("涨跌幅") or "").strip()
                try:
                    ret = float(ret_str)
                except ValueError:
                    continue
                if ret <= 0:
                    continue
                out_rows.append({
                    "会议名称": (row.get("会议名称") or "").strip(),
                    "T日": (row.get("T日") or "").strip(),
                    "窗口": w,
                    "窗口开始": (row.get("窗口开始") or "").strip(),
                    "窗口结束": (row.get("窗口结束") or "").strip(),
                    "市场涨跌幅": row.get("市场涨跌幅", ""),
                    "上证指数涨跌幅": row.get("上证指数涨跌幅", ""),
                    "行业名称": (row.get("行业名称") or "").strip(),
                    "上涨幅度": round(ret, 6),
                    "差值": row.get("差值", ""),
                })

    # 在每组（会议+窗口）之间插空行，避免行业挤在一起难读
    empty_row = {k: "" for k in out_fields}
    rows_to_write = []
    prev_key = None
    for r in out_rows:
        key = (r["会议名称"], r["T日"], r["窗口"])
        if prev_key is not None and key != prev_key:
            rows_to_write.append(empty_row)
        prev_key = key
        rows_to_write.append(r)

    path_out = dir_data / "各窗口上涨行业.csv"
    with open(path_out, "w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=out_fields)
        writer.writeheader()
        writer.writerows(rows_to_write)
    print(f"已写入 {path_out}，共 {len(out_rows)} 条（仅上涨行业），组间已插空行。")


if __name__ == "__main__":
    run()
